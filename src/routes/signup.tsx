...
function SignupScreen() {
  ...
  const handleSignup = async () => {
    ...
    const { data, error: authError } = await supabase.auth.signUp({ phone, password, options: { data: { full_name: name.trim() } } });
    if (authError) { setError(authError.message); setLoading(false); return; }
    if (data.user && data.session) {
      await supabase.from("profiles").upsert({ id: data.user.id, full_name: name.trim(), phone });
      localStorage.setItem("old-touch-logged-in", "true");
      navigate({ to: "/setup" });
      return;
    }
    setError("Account created. Please complete phone verification, then log in.");
    setLoading(false);
  };
